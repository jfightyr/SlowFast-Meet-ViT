...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def evaluate(self):
        ...
        """
        scores,tp_fp_labels,(per_class_ap, mean_ap, _, _, per_class_corloc, mean_corloc) = (
        ...
        return pascal_metrics,scores,tp_fp_labels
...
class ObjectDetectionEvaluation(object):
    ...
    def evaluate(self):
        ...
        for class_index in range(self.num_class):   # 输入的是predict.csv
            if self.num_gt_instances_per_class[class_index] == 0:    # 如果某个类别没有 ground truth 样本，则跳过该类别
                continue
            if not self.scores_per_class[class_index]:  # 如果该类别没有检测结果，则创建空的数组
                scores = np.array([], dtype=float)
                tp_fp_labels = np.array([], dtype=bool)
            else:   # 否则，拼接该类别的所有检测结果
                scores = np.concatenate(self.scores_per_class[class_index]) # 该类别的score全部写入
                tp_fp_labels = np.concatenate(self.tp_fp_labels_per_class[class_index])
            if self.use_weighted_mean_ap:
                all_scores = np.append(all_scores, scores)
                all_tp_fp_labels = np.append(all_tp_fp_labels, tp_fp_labels)
            precision, recall = metrics.compute_precision_recall(    # 计算该类别的精度和召回率
        ...
        return scores,tp_fp_labels,ObjectDetectionEvalMetrics(
