...
class PerImageEvaluation(object):
    ...
    def compute_object_detection_metrics(
        ...
        这段代码是用于评估从单个图像中检测到的目标实例的情况，
        将检测结果分类为真正例（true positive）、假正例（false positive）或被忽略的情况。
    ...
    def _get_overlaps_and_scores_box_mode(
        ...
        iou = np_box_list_ops.iou(detected_boxlist, gt_non_group_of_boxlist)    # bbox送进去计算iou
