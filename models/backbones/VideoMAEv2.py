...
class PatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.proj = nn.Conv3d(
            ...
            # [1, 3, 32, 224, 224] -> [1, 1408, 16, 16, 16] ->

    def forward(self, x, **kwargs):
        B, C, T, H, W = x.shape    #([1, 3, 32, 224, 224])
        assert H == self.img_size[0] and W == self.img_size[
            1], f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        # b, c, l -> b, l, c
        x = self.proj(x).flatten(2).transpose(1, 2)
        # [1, 3, 32, 224, 224] -> [1, 1408, 16, 16, 16] -> [1, 1408, 4096] -> [1, 4096, 1408]
        # [B, C, N] -> [B, N, C] used for attention
        return x  # (B, num_patches, embed_dim)
                  # torch.Size([1, 4096, 1408]), giant
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        B = x.size(0)  # torch.Size([1, 3, 32, 224, 224])

        x = self.patch_embed(x) # torch.Size([1, 3, 32, 224, 224])

        if self.pos_embed is not None:
            x = x + self.pos_embed.expand(B, -1, -1).type_as(x).to(
                x.device).clone().detach()
        x = self.pos_drop(x)   # torch.Size([1, 4096, 1408])
