...
from utils import *
from multi_cap_mAP import cal_cap_map
...
def main(local_rank, args):
    ...
    net.cuda()
    net = DistributedDataParallel(net, device_ids=[local_rank], broadcast_buffers=False, find_unused_parameters=False)
    ...
    if not opt.get('evaluate', False):
        ...
        # train_data[0]
        # dict_keys(['clip', 'aug_info', 'label', 'video_name', 'mid_time'])
    ...
    if opt.get('pretrain', None) is not None:
        load_fortrain(opt.pretrain, net)

    begin_epoch = 1
    if opt.get('resume_path', None) is not None:
        print("loading ckpt for resume.")
...
def val_epoch(epoch, data_loader, model, criterion, act_func,
    ...
    if rank == 0:
        ...
        result_txt = os.path.join(opt.result_path, 'cap_mAP_records_multi%d.txt'%epoch)
        ...
        if opt.val.get('eval_mAP', None) is not None:
            cap_min = opt.cap_min
            cap_max = opt.cap_max
            cap_range = range(cap_min,cap_max)
            mAP,max_cap = cal_cap_map(result_file,result_txt,cap_range)
...
# make sure: device, address, result_path, batch_size, train_epoch & val_epoch, acar.head.width
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PyTorch AVA Training and Evaluation')
    parser.add_argument('--config', type=str, 
    default=r"./configs/VITSF_ACAR_HR2O_CW_train_resume.yaml")
    ...
    parser.add_argument('--master_port', type=int, default=31775)
